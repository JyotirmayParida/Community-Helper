  ...
  if (loading) {
    return (
      <div className="py-24 flex flex-col items-center justify-center gap-2">
        ...
        <span className="text-xs font-mono tracking-wider text-stone-500 uppercase">Verifying dispatch clearance...</span>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="border-b border-[#1C1A17] pb-4 space-y-2">
        <div className="flex items-center gap-2 text-stone-600">
          ...
          <span className="font-mono text-xs uppercase tracking-widest">Incident Dispatcher</span>
        </div>
      ...
      <div className="border border-[#1C1A17] p-8 bg-[#FAF9F6] space-y-6 relative rounded-sm shadow-[3px_3px_0px_0px_#1C1A17]">
        <div className="space-y-4">
          <h3 className="font-serif text-xl font-bold italic text-stone-800">
            Dispatch Queue Offline for Maintenance
          </h3>
          <p className="text-stone-700 text-sm leading-relaxed font-sans">
            You have successfully cleared validation. The active reporting engine is currently staged. 
            In the upcoming dispatch cycle, this portal will support real-time category tagging, 
            instant duplicate checking, automated division routing, and image-based validation powered by Gemini AI.
          </p>
        </div>
        ...
        <div className="border-t border-[#1C1A17]/10 pt-4 flex items-start gap-2.5 font-mono text-[11px] text-stone-500 uppercase tracking-wider">
          ...
          <span>STAGED MODULE — SYSTEM STANDBY</span>
        </div>
      </div>
    </div>
  );
